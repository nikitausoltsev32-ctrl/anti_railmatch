...
export default function App() {
    ...
    // 2b. ГЛОБАЛЬНЫЙ ПЕРЕХВАТЧИК ОШИБОК → error_logs
    useEffect(() => {
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div onClick={() => requireAuth(() => setView('profile'))} className="flex items-center gap-3 cursor-pointer pl-6 border-l dark:border-slate-800 group">
                            <div className="text-right hidden sm:block">
                                ...
                                <div className="text-[10px] uppercase font-black text-slate-400 tracking-widest">{userProfile?.role === 'shipper' ? 'Отправитель' : userProfile?.role === 'owner' ? 'Владелец' : userProfile?.role === 'developer' ? 'Разработчик' : 'Гость'}</div>
                            </div>
