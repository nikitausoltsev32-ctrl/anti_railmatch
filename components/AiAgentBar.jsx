...
export default function AiAgentBar({ onSearch, activeFilters, onClearFilter, userRole }) {
    ...
    return (
        <div className="mb-8">
            ...
            <div className="bg-white dark:bg-[#111827] p-1.5 rounded-2xl sm:rounded-[24px] border border-blue-100 dark:border-slate-800 shadow-xl shadow-blue-500/5 flex items-center gap-1.5 sm:gap-2 focus-within:shadow-[0_0_30px_rgba(37,99,235,0.15)] focus-within:border-blue-300 dark:focus-within:border-blue-700 focus-within:-translate-y-1 hover:-translate-y-0.5 transition-all duration-500 group overflow-hidden bg-gradient-to-r from-transparent via-blue-50/10 to-transparent bg-[length:200%_100%] focus-within:animate-[shimmer_2s_infinite]">
                <div className="pl-3 sm:pl-4 pr-2 sm:pr-3 py-3 sm:py-3.5 bg-blue-50 dark:bg-blue-900/30 rounded-xl sm:rounded-[18px] ml-0.5 sm:ml-1 text-blue-600 dark:text-blue-400 flex items-center gap-2 border border-blue-100/50 group-focus-within:bg-blue-600 group-focus-within:text-white group-focus-within:border-blue-500 transition-all duration-500 shadow-inner">
                    <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 group-focus-within:animate-spin" />
                ...
                <input
                    ...
                    placeholder={clarification ? 'Ответ...' : "Найти или создать..."}
                    className="flex-1 py-3 sm:py-4 px-2 sm:px-3 outline-none text-slate-700 dark:text-white font-semibold placeholder:text-slate-300 dark:placeholder:text-slate-600 bg-transparent transition-all duration-300 text-sm sm:text-base min-w-0"
                    autoFocus={!!clarification}
                />
                <button
                    onClick={handleSearch}
                    className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 sm:px-8 py-3 sm:py-4 mr-0.5 sm:mr-1 rounded-xl sm:rounded-[18px] font-bold flex items-center gap-2 shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:scale-105 active:scale-95 hover:-translate-y-1 transition-all duration-300 group-focus-within:shadow-[0_0_20px_rgba(37,99,235,0.4)] shrink-0"
                >
